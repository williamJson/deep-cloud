...
public class Sequence {
    ...
    /**
     * dataCenterId占用的位数：2
     * dataCenterId和workerId的取值范围可以根据实际情况进行选择
     * 两者综合不超过10位即可
     **/
     ...
     *
     * @param dataCenterId   数据中心ID,数据范围为0~3 这里作者取值范围有笔误，大家注意
    ...
    public synchronized Long nextId() {
        ...
        if (currentTimestamp < lastTimestamp) {
            ...
            try {
                ...
                //这里使用wait(timeOut)让线程休眠并且释放锁
                //一般我们会把wait和while结合在一起使用，因为使用if和wait结合存现判断条件“丢失”的问题，但这里不需要
                //后面紧跟再次判断<所以wait的用法还是要跟实际的场景相结合>
                this.wait(offset << 1);
            } catch (Exception e) {
    ...
    public synchronized String dateId() {
        long currentTimestamp = this.timeGen();
        ...
        String dateTime = sdf.format(new Date(lastTimestamp));
